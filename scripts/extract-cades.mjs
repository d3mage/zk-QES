...
function extractCMSFromPDF(pdfPath) {
...
function extractByteRangeData(pdfPath) {
    const pdf = fs.readFileSync(pdfPath);
    const pdfStr = pdf.toString('latin1');
...
async function main() {
    try {
        ...
        // Build SET OF from attributes for signing
        const attrsForSigning = new asn1js.Set({
            value: signerInfo.signedAttrs.attributes.map(attr => attr.toSchema())
        });
        ...
        const messageDigestAttr = signerInfo.signedAttrs.attributes.find(
            attr => attr.type === '1.2.840.113549.1.9.4' // messageDigest OID
        );
        ...
        const sigSequence = sigAsn1.result;
        const rValue = sigSequence.valueBlock.value[0].valueBlock.valueHexView;
        const sValue = sigSequence.valueBlock.value[1].valueBlock.valueHexView;
        ...
        const pubKeyPem = `-----BEGIN PUBLIC KEY-----\n${Buffer.from(pubKeyInfo.toSchema().toBER()).toString('base64').match(/.{1,64}/g).join('\n')}\n-----END PUBLIC KEY-----`;
